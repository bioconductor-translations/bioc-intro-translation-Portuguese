...
- Produzir gráficos de dispersão, boxplots, gráficos de linhas, etc. utilizando ggplot.
- Definir configurações do plot universal.
- Descrever o que é faceting e aplicar faceting no ggplot.
- Modifique a estética de um gráfico do ggplot (incluindo rótulos e cores dos eixos).
- Construir gráficos complexos e personalizados a partir de dados num data.frame.
...
Começamos por carregar os pacotes necessários. **`ggplot2`** está incluído no pacote **`tidyverse`**.
...
muitas representações de dados disponíveis no pacote. Os seguintes tutoriais em vídeo
...
a partir de dados em um quadro de dados. Fornece uma interface
...
o `ggplot2` é a *Gramática de Gráficos* (@Wilkinson:2005). Utilizando esta abordagem, apenas necessitamos de alterações mínimas se os dados subjacentes mudarem ou
...
> A ideia por detrás da Gramática de Gráficos é que pode construir todos os gráficos
...
[^three\_comp\_ggplot2]: Fonte: [Data Visualization Cheat Sheet](https://raw.githubusercontent.com/rstudio/cheatsheets/main/data-visualization.pdf).

Para construir um ggplot, vamos utilizar o seguinte modelo básico que pode ser
...
- adicionar '**geoms**' - geometrias, ou representações gráficas dos
  dados no gráfico (pontos, linhas, barras). `ggplot2` offers many
